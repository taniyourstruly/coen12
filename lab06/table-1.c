 ...
 * Complexity: O(n) 
 *
 * Description: finding the pivot place in the array where the wall is
 */

static int pivot(SET*sp, void**d,  int l, int r)
{
	assert(sp!=NULL);
	void*p = d[r];
	...
	{	
		...
		// if the pivot is smaller, swap
		if ((*sp->compare)(d[j], p) <= 0)
		{
			i++;
			temp = d[i];
			d[i]=d[j];
 ...
 * Complexity: O(nlogn)
 *
 * Description: calls quicksort for the parts of the wall
 */
static void quicksort(SET*sp, void**d, int l, int r)
{
	assert(sp!=NULL);
	if(l<r)
	{
		int p = pivot(sp, d, l, r);
		quicksort(sp, d, l, p-1);
		quicksort(sp, d, p+1, r);
	}
...
{
    assert(sp!=NULL);
    void**new = malloc(sizeof(void*)*sp->count);
    assert(new!=NULL);
    int i, j;
